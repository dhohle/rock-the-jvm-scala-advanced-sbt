...
object TypeClasses extends App {
  ...
  /*
  1 - lost type safety
  2 - need to modify code every time (something changes)
  ...
  println(HTMLSerializer.serialize(42)) //(implicit IntSerializer))
  println(HTMLSerializer.serialize(john)) //(implicit UserSerializer))

  // access to the entire type class interface
  println(HTMLSerializer[User].serialize(john)) //(implicit UserSerializer))
